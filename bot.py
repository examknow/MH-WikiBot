...
lastgreeter = ''
greetings = ['Hello {}!', 'Hi {}!', 'Hello there {}!', 'Hi there {}!',
             'Hey {}!']
...
#		bot.send_message(channel, "An error occured.")
    if message.lower().startswith('!userinfo'):
        ...
        try:
            ...
            for u in USERS:
                try:
                    if str(u['blockid']) != '':
                        bot.send_message(channel, str(u['name'])
                                + ' (Blocked) has made '
                                + str(u['editcount']) + ' edits on '
                                + wiki + 'wiki')
                except KeyError:
                    bot.send_message(channel, str(u['name'])
                            + ' has made ' + str(u['editcount'])
                            + ' edits on ' + wiki + 'wiki.')
        except ValueError:
            ...
            bot.send_message(channel,
                             'An error occured. Did you type the wiki incorrectly? Does the user exist?'
                             )
    if message.lower().startswith('!globaluser'):
        ...
        try:
            ...
            for u in GLOBALUSERINFO:
                try:
                    if u['locked'] == '':
                        bot.send_message(channel, u['name']
                                + ' (Globally Locked) has made '
                                + u['editcount']
                                + ' edits on all Miraheze wikis')
                except KeyError:
    ...
    if message.lower().startswith('!blockuser') and senderhost \
        ...
        if len(arg) == 3:
            ...
            bot.send_message(channel,
                             'Syntax is !blockuser <wiki> <user> <reason>'
                             )
            return
        ...
        try:
            ...
            bot.send_message(channel,
                             'Catostrophic Error! Unable to login to the wiki.'
                             )
...
# Step 4: POST request to block user
        ...
        try:
            ...
            bot.send_message(channel,
                             'Block request sent. You may want to check https://'
                              + wiki
                             + '.miraheze.org/wiki/Special:Log?type=block&page='
                              + user
                             + ' to confirm that the block worked.')
            bot.send_message('#SigmaBot-logs', sender + ' ('
                             + senderhost + ') just blocked ' + user
                             + ' on ' + wiki + 'wiki for ' + reason)
        except:
            bot.send_message(channel,
                             'An unexpected error occured. Did you type the wiki or user incorrectly? Do I have admin rights on that wiki?'
                             )
            return
...
# Step 4: POST request to block user
        ...
        try:
            ...
            DATA = R.json()
            bot.send_message(channel,
                             'Unblock request sent. You may want to check https://'
                              + wiki
                             + '.miraheze.org/wiki/Special:Log?type=block&page='
                              + user
                             + ' to confirm that the unblock worked.')
            bot.send_message('#SigmaBot-logs', sender + ' ('
                             + senderhost + ') just (un)blocked '
                             + user + ' on ' + wiki + 'wiki for '
                             + reason)
        except:
            bot.send_message(channel,
                             'An unexpected error occured. Did you type the wiki or user incorrectly? Do I have admin rights on that wiki?'
    ...
    if message.lower().startswith('!delete') and senderhost in stewards:
        ...
        if len(arg) == 3:
            try:
                ...
                bot.send_message(channel,
                                 'Syntax is !delete <wiki> <page> <reason>'
                                 )
                return
        elif len(arg) > 3:
            try:
        ...
        try:
            ...
            DATA = R.json()
            bot.send_message(channel,
                             'The delete request was sent. You should check the wiki to make sure the page was deleted.'
                             )
            bot.send_message('#SigmaBot-logs', sender + ' ('
                             + senderhost + ') just deleted ' + page
                             + ' on ' + wiki + 'wiki for ' + reason)
        except:
            bot.send_message(channel,
                             'An unexpected error occured. Did you type the wiki or page incorrectly? Do I have admin rights on that wiki?'
                             )
...
# Step 4: POST request to block user
        ...
        try:
            ...
            DATA = R.json()
            bot.send_message(channel,
                             'Logged message at https://test.miraheze.org/wiki/TestWiki:Admin_Action_Log'
                             )
        except:
            bot.send_message(channel,
                             'An unexpected error occured. Do I have edit rights on that wiki?'
